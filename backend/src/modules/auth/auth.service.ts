...
export class AuthService {
    public async register(registerData: RegisterDto) {
        ...
        if (existingUser) {
            throw new BadRequestException('User already exists wi ', ErrorCode.AUTH_EMAIL_ALREADY_EXISTS);
        }
    ...
    public async login(loginData: LoginDto) {
        const { email, password, userArgent } = loginData;

        const user = await UserModel.findOne({ email: email });

        if (!user) {
            throw new BadRequestException('Invalid email or password provid provided', ErrorCode.AUTH_USER_NOT_FOUND);
        }
    ...
    public async refreshToken(refreshToken: string) {
        ...
        if (!session) {
            throw new UnauthorizedException('Sesion does not exist');
        }
